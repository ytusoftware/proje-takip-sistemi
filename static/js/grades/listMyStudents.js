function pass_func(template_values_curr) {
    $(document).ready(function() {
            ...
            student_list.forEach(function(student, index) {
                    $("#student-list tbody").append('<tr> <th scope = "row">' + (student[0]).toString() + '</th> <td>' + student[1] + ' </td> <td>' + student[2] + '</td> <td>' + student[3] + '</td> <td>'+student[4]+ '</td><td><a href="#my_modal" class="identifyingClass" data-id= '+"'"+(student[0]).toString()+"'"+' data-toggle="modal" data-target="#myModal">Not Girişi Yap</a></td> </tr>');
            });
            $(function () {
            ...
            updated_prev_page_url = "/Grades/NotGirisSayfasi?page=" + (init_page_num-1).toString();
            $("#pageprev").attr("href", updated_prev_page_url);

            updated_next_page_url = "/Grades/NotGirisSayfasi?page=" + (init_page_num+1).toString();
